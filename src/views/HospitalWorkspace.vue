...
<template>
  <PageHeader eyebrow="Hospital intake workspace" title="国内医院国际患者承接中心" subtitle="从接诊评估到床位、检查、治疗、费用和出院交接">
    <button class="primary-button" @click="store.acceptHospital()">确认承接并预留床位</button>
  </PageHeader>
  ...
  <div class="grid-2">
    <SectionCard title="今日重点承接申请" subtitle="从待接诊队列中选择的高优先级申请">
      <div class="info-list">
        ...
        <div class="info-row"><span>计划入院</span><b>{{ store.state.treatment.admissionDate }}</b></div>
        <div class="info-row"><span>床位</span><b>{{ store.state.treatment.bed }}</b></div>
        <div class="info-row"><span>费用预估</span><b>{{ store.state.treatment.estimatedCost }}</b></div>
        <div class="info-row"><span>接诊状态</span><span class="status-pill" :class="store.state.treatment.status === 'accepted' ? 'done' : 'pending'">{{ store.state.treatment.status === 'accepted' ? '已承接' : '待确认' }}</span></div>
      </div>
  ...
  <SectionCard title="拟定诊疗日程" subtitle="确认承接后同步患者端与中马运营团队">
    <div class="stage-list">
      <div v-for="(item, i) in store.state.treatment.schedule" :key="item.date" class="stage-card">
  ...
  <div class="grid-equal">
    ...
    <SectionCard title="费用状态">
      <div class="info-list">
        <div class="info-row"><span>预估总费用</span><b>{{ store.state.treatment.estimatedCost }}</b></div>
        <div class="info-row"><span>已支付</span><b>¥{{ store.state.treatment.paid.toLocaleString() }}</b></div>
