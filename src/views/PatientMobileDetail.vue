...
const stage = computed(() => store.activeFollowup.stages.find(item => item.id === route.params.id))
</script>
<template>
  <div class="patient-mobile-detail">
    ...
    <main v-if="type==='record'"><div class="mobile-detail-file">PDF</div><h1>{{ doc?.name }}</h1><p>{{ doc?.type }} · {{ doc?.source }} · v{{ doc?.version }}</p><section><h2>文件状态</h2><div class="mobile-detail-row"><span>核验状态</span><b>{{ doc?.medicalVerification }}</b></div><div class="mobile-detail-row"><span>文件语言</span><b>{{ doc?.language }}</b></div><div class="mobile-detail-row"><span>授权范围</span><b>{{ doc?.authorizationScopes.join(' / ') }}</b></div></section><button class="patient-primary-action" @click="message=store.accessDocument(doc.id, store.activePatient.name).message">预览文件</button></main>
    <main v-else-if="type==='plan'"><div class="mobile-detail-logo">H</div><h1>{{ store.activeTreatment.hospital || '待专家确认医院' }}</h1><p>国际医疗中心 · {{ store.activeTreatment.department || '待确认' }}</p><section><h2>接诊信息</h2><div class="mobile-detail-row"><span>接诊医生</span><b>{{ store.activeTreatment.doctor || '待确认' }}</b></div><div class="mobile-detail-row"><span>预计入院</span><b>{{ store.activeTreatment.admissionDate || '待确认' }}</b></div><div class="mobile-detail-row"><span>费用预估</span><b>{{ store.activeTreatment.estimatedCost }}</b></div><div class="mobile-detail-row"><span>床位</span><b>{{ store.activeTreatment.bed }}</b></div></section><section class="mobile-detail-note"><b>温馨提示</b><p>最终诊疗安排以到院检查和 MDT 结论为准。</p></section></main>
